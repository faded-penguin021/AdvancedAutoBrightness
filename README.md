...
* Go darker than Android minimum witn Super Dimming using privileged methods (Reduce Bright Colors) or overlays
...
In AAB, manual brightness adjustments can be used as training data **if** you enable override detection. Each manual adjustments gets logged the moment you let go of the brightness slider. When you have enough data (a minimum of 9 data points) spread across different lighting conditions, go to the Brightness Graph and hit the Suggest values button.
...
_Note: The balance between responsiveness and stability is always a trade off between jitter and responsiveness. But you are in control! 
_
### Hybrid Dimming
...
_Note: if no location and/or date are set, AAB will try to use your current location and the current date. 
_
### Target audience
...
Note: in it's current architecture the main brightness loop is **not much faster** due to inter-process context switching between native Tasker and Java.
#### Battery efficiency
